class Site::Profile::MyDataController < Site::ProfileController
  ...
  def update
    if @profile_member.update(params_profile_member)
      redirect_to edit_site_profile_my_datum_path(current_member.id), notice: "Os dados foram atualizados com sucesso!"
    else
      render :edit
    end
  end
  ...
  private
    ...
    def params_profile_member
        params.require(:profile_member).permit(:id, :first_name, :second_name, :birth_date)
    end
